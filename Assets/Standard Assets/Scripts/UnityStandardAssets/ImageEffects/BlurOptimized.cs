...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class BlurOptimized : PostEffectsBase
	{
		...
		public void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			int arg_BB_0 = (this.blurType != BlurOptimized.BlurType.StandardGauss) ? 2 : 0;
			for (int i = 0; i < this.blurIterations; i++)
			{
				float num2 = (float)i * 1f;
				this.blurMaterial.SetVector("_Parameter", new Vector4(this.blurSize * num + num2, -this.blurSize * num - num2, 0f, 0f));
				RenderTexture temporary = RenderTexture.GetTemporary(width, height, 0, source.format);
				temporary.filterMode = FilterMode.Bilinear;
				Graphics.Blit(renderTexture, temporary, this.blurMaterial);
